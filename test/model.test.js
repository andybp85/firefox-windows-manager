...
import {
    ...
    hostOf,
    insertIndexAmong,
    moveWindowAmongColumns,
    sortWindowsByOrder,
    tabsToUnloadAllButActive,
} from '../src/model.js'
...
const tab = (id, windowId, extra = {}) => ({
    ...
    index: id,
    title: `Tab ${id}`,
    url: `https://site${id}.example/page`,
    windowId,
    ...extra,
})
...
test('buildModel nests groups and ungrouped tabs under their window, ordered by tab index', () => {
    const windows = [win(1)]
    const tabs = [tab(10, 1, { groupId: 100, index: 0 }), tab(11, 1, { groupId: -1, index: 1 }), tab(12, 1, { groupId: 100, index: 2 })]
    ...
    assert.deepEqual(
        w.groups[0].tabs.map(t => t.id),
        [10, 12],
    )
    assert.deepEqual(
        w.ungrouped.map(t => t.id),
        [11],
    )
...
test('buildModel drops group references with no matching group definition into ungrouped', () => {
    ...
    assert.deepEqual(
        model.windows[0].ungrouped.map(t => t.id),
        [10],
    )
})

test('deriveCounts totals windows, groups, and tabs', () => {
    const model = buildModel([win(1), win(2)], [tab(10, 1, { groupId: 100 }), tab(11, 1), tab(20, 2)], [group(100, 1)], {})
...
test('allTabsOf returns grouped tabs then ungrouped tabs', () => {
    const model = buildModel([win(1)], [tab(10, 1, { groupId: 100, index: 0 }), tab(11, 1, { index: 1 })], [group(100, 1)], {})
    assert.deepEqual(
        allTabsOf(model.windows[0]).map(t => t.id),
        [10, 11],
    )
})
...
test('tabsToUnloadAllButActive skips active and already-discarded tabs, scoped to all', () => {
    const model = buildModel(
        [win(1), win(2)],
        [tab(10, 1, { active: true }), tab(11, 1, { discarded: true }), tab(12, 1), tab(20, 2, { active: true }), tab(21, 2)],
        [],
        {},
    )
    assert.deepEqual(
        tabsToUnloadAllButActive(model, 'all').sort((a, b) => a - b),
        [12, 21],
    )
})

test('tabsToUnloadAllButActive scoped to one window ignores other windows', () => {
    const model = buildModel([win(1), win(2)], [tab(10, 1, { active: true }), tab(11, 1), tab(20, 2), tab(21, 2)], [], {})
...
test('sortWindowsByOrder orders by stored order, unordered windows last by id', () => {
    ...
    assert.deepEqual(
        sortWindowsByOrder(ws, { 10: 2, 20: 0, 30: 1 }).map(w => w.id),
        [20, 30, 10],
    )
    assert.deepEqual(
        sortWindowsByOrder(ws, {}).map(w => w.id),
        [10, 20, 30],
    )
    assert.deepEqual(
        sortWindowsByOrder(ws, { 30: 0 }).map(w => w.id),
        [30, 10, 20],
    )
})
