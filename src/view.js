...
function renderTab(tab) {
    ...
    controls.append(iconButton('unload-tab', '⏏', 'Unload', { tabId: tab.id }), button('close-tab', '✕', { tabId: tab.id }))
...
function renderGroup(group) {
    ...
    header.append(el('span', 'group-title', group.title || 'Group'), button('close-group', 'Close group', { groupId: group.id }))

    const body = el('div', 'group-tabs')
    for (const tab of group.tabs) body.append(renderTab(tab))
...
function renderWindow(windowVM) {
    ...
    if (windowVM.focused) section.classList.add('focused')
    ...
    header.append(grip, name, el('span', 'window-tabcount', `${windowVM.tabCount} tabs`), actions)

    const body = el('div', 'window-body')
    for (const group of windowVM.groups) body.append(renderGroup(group))
    for (const tab of windowVM.ungrouped) body.append(renderTab(tab))
...
export function render(model, options = {}) {
    ...
    if (!supported) root.append(el('p', 'notice', 'Tab groups need Firefox 139+ — showing windows and tabs only.'))
